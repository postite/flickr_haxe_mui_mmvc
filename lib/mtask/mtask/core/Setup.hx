...
/**
	Defines tasks used for initial setup of mtask.
...
class Setup extends mtask.core.Module
{
	...
	/**
		Removes all builds from ${build.home}bin
	**/
	@task function reset()
	{
		rm("${build.home}bin");
	...
	**/
	@task function config(?key:String, ?value:String, ?global:Bool=false)
	{
		...
		{
			if (key == null) error("Cannot configure null key");
			...
			else if (!build.isProject)
			{
				error("Cannot set local config outside of project.");
			}
