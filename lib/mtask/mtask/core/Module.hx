...
/**
	The base class for an mtask module, defining common utility methods.
...
class Module
{
	...
	**/
	...
	/**
		Invokes a task that matches the provided arg string, throwing an 
	...
	**/
	...
	/**
		Invokes a task that matches the provided arguments, throwing an 
		exception if none is found.
	**/
	public function invokeArgs(args:Array<String>)
	{
		build.invokeArgs(args);
	}
	...
	**/
	...
	{
		Console.log(replaceArgs(message));
	}

	/**
		Print a warning to the console and optionally exit the application.
	**/
	function warn(message:String, quit:Bool = false)
	{
		Console.log(mconsole.Style.yellow(replaceArgs(message)));
		if (quit) Sys.exit(1);
	}
	...
	/**
		Raise an error.
	**/
	function error(message:String, ?stack:Array<StackItem>)
	{
		throw new Error(replaceArgs(message), stack);
	}
	...
	**/
	function cmd(command:String, args:Array<String>, ?print:Bool=true):String
	{
		command = replaceArgs(command);
		for (i in 0...args.length) args[i] = replaceArgs(args[i]);
		return mtask.core.Process.run(replaceArgs(command), args, print);
	}
	...
	**/
	function openURL(path:String)
	{
		...
		// When using a virtual host url we don't need to append the file path
		if (build.env.get("localhost.vhost") == true)
		{
			open(build.env.get("localhost.url"));
			return;
		}

		if (root == "" || !StringTools.startsWith(path.toLowerCase(), root.toLowerCase()))
		{
