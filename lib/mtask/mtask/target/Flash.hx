...
/**
	An Adobe Flash target embedded in an HTML template.
**/
class Flash extends App
{
	...
	override function processAssetLibrary(id:String, files:Array<TargetFile>, variant)
	{
		super.processAssetLibrary(id, files, variant);
		
		var hxmls = [];
		var swf = msys.Path.join([path, "asset", variant, id + ".swf"]);
		hxmls.push("-swf " + swf);
		hxmls.push("-lib mtask");

		for (file in files)
		{
			var absolute = msys.File.unixPath(file.absolute);
			// file.local is already in unix format (hence no Path.split)
			var path = file.local.split("/").join(".");
			hxmls.push("--macro mtask.target.ImageMacro.embed('"+absolute+"','asset."+path+"')");
		}
